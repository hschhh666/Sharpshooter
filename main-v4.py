...
from grabScreen import grabTwoWindowsThread
...
def friend_judge(img, x, allFriend = False): # True 朋友 False 敌人
    ...
    b,g,r = img[170,170,0],img[170,170,1],img[170,170,2]
    ...
    for i in range(20,lenth):
        ...
        if abs(img[y,x,0] -b) < 50 and abs(img[y,x,1] - g)<50 and (img[y,x,2] - r)<50:
            return True
        # if img[y,x,2] > 180 and img[y,x,0] < 50 and img[y,x,1] < 50:
        #     return False
    return False

def detection_and_operating(auto_attack_flag, queue,lock): # 默认不自动攻击
    ...
    while 1:
        ...
        lock.acquire()
        if queue.qsize():
            little_map, img, thread_time = queue.get()
        else:
            lock.release()
            continue
        lock.release()
        ...
        print('Grab img time: %.2fms, fre: %.2fhz'%(1000*(grab_time),1/(grab_time+0.0001)))
        ...
        textOnImg = 'grab thread: %.0fms grab: %.0fms detection: %.0fms vis: %.0fms click: %.0fms'%(1000*thread_time, 1000*grab_time,1000*detection_time,1000*vis_time,1000*click_time)
        cv2.putText(img,textOnImg,(20,20),1,1,(0,0,255,2))
        textOnImg = 'total: %.0fms AI: %d'%(1000*total_time,auto_attack_flag.value)
        cv2.putText(img,textOnImg,(20,40),1,1,(0,0,255,2))
        cv2.imshow('img',img)
        ...
        if not set_window:
            cv2.moveWindow("little", 2560, 400)
            set_window = True
        cv2.waitKey(1)
...
if __name__ == '__main__':
    ...
    queue = multiprocessing.Queue() # 保证队列中始终只有当前的截屏
    p_img = multiprocessing.Process(target=grabTwoWindowsThread,args=(queue,lock))

    auto_attack_flag = multiprocessing.Value('b', 0) # 是否自动攻击的标志位
    p = multiprocessing.Process(target=detection_and_operating, args=(auto_attack_flag,queue,lock))
    p.start()
